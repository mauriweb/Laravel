                ...
                <table width="80%" class="text_left table_gran"  style="margin-left:auto; margin-right:auto; position:relative;">
                    <thead>
                        <tr class="black">
                            ...
                            <td style="width:100%; text-align:center;" colspan="2"><h1>FORMULA AJUSTADA A PRODUCCIÓN</h1></td>
                            <?php }else{ ?>
                            <td style="width:100%; text-align:center;" colspan="2"><h1>FORMULA <?php echo $enlucido. ' Maquina: '.$size; ?></h1></td>
                            <?php } ?>
                        </tr>
					</thead>
                    <tbody>
                            ...
                            <tr>
                                <td style="width:50%;"><small>Equivalencia:</small><strong>
                                        <?php $coma='';?>
                                    @foreach($formula->FormulasEquivalencia as $eq)
                                        @if($eq->display==1)
                                            {{ $coma.$eq->equivalencia }} <?php if($eq->codigo!='')echo '('.$eq->codigo.')'; ?>
                                        @endif
                                    <?php $coma=', ';?>
                                    @endforeach
                                    </strong>	</td>
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td><small class="nombre">{{$formDet->Producto->nombreClave}}</small>
                                    <input type="hidden" class="cantidad" value="{{$formDet->cantidad}}"></td>
